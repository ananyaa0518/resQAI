...
export default function AdminDashboard() {
  ...
  const verifyReport = async (reportId, status, notes = "") => {
    try {
      ...
      await axios.post(`/api/reports/${reportId}/verify`, {
        status,
        notes
      }, {
        headers: {
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        <span className="px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded-full">
                          {report.disasterType} ({Math.round(report.confidence * 100)}%)
                        </span>
                      </div>
                      <p className="text-gray-700 mb-2">{report.description}</p>
                      <div className="text-sm text-gray-500 space-y-1">
                        <p><strong>Location:</strong> {report.location?.coordinates?.[1]?.toFixed(4)}, {report.location?.coordinates?.[0]?.toFixed(4)}</p>
                        <p><strong>Reported by:</strong> {report.reportedBy?.name} ({report.reportedBy?.phoneNumber})</p>
                        <p><strong>Time:</strong> {new Date(report.createdAt).toLocaleString()}</p>
                        {report.verificationNotes && (
                          <p><strong>Verification Notes:</strong> {report.verificationNotes}</p>
                        )}
                      </div>
                    </div>
                  </div>
